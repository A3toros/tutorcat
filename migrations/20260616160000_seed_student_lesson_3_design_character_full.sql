-- Lesson 3: Design Your Own Character — single source of truth seed
-- Safe to re-run: deletes lesson by slug then re-inserts (CASCADE).
-- Starts INACTIVE — admin can activate when ready.
...
INSERT INTO student_lessons (
  ...
  'design-your-own-character',
  35,
  'Students design a character, describe appearance and personality, and speak about their character.',
  $gf${
    "appearance": {
...
CROSS JOIN (VALUES
  ...
  (
    ...
    '{"group_by_category": true, "show_thai": false, "tap_thai_translation": true, "study_seconds": 120}'::text,
    240
  ),
  (
    3,
...
Build three sentences about your character using the dropdowns.$desc$,
  ...
  (
    ...
    '{"duration_seconds": 30, "prompts": ["Describe your character''s appearance.", "What is your character''s personality?", "What does your character like doing?", "Would you be friends with your character? Why?"]}'::text,
    300
  ),
  (
    9,
...
-- Personality vocabulary (activity 4) — positive/neutral/negative + Thai
INSERT INTO student_vocabulary_items (activity_id, english_word, thai_translation, category, image_url, sort_order)
SELECT la.id, v.word, v.thai, v.category, v.image_url, v.sort_order
FROM student_lesson_activities la
JOIN student_lessons sl ON sl.id = la.student_lesson_id
...
-- Exit ticket: picture → word (activity 9) — graded
